...
class FixationDetection(RenaScript):
    ...
    def init(self):
        ...
        self.gap_filled_xyz_buffer = DataBuffer()
    ...
    def loop(self):
        ...
        if self.gaze_stream_name in self.inputs.keys():
            ...
            if gaze_status[-1] == self.gaze_status['valid']:  # and starting_invalid_duration > self.max_gap_time:  # if the sequence starts out invalid, we must wait until the end of the invalid
                ...
                self.gap_filled_xyz_buffer.update_buffer({'stream_name': 'gap_filled_xyz', 'frames': gap_filled_xyz, 'timestamps': gaze_timestamps})  # add the gap filled data to the buffer, so we can use it for fixation detection
                ...
                self.gaze_timestamp_head = gaze_timestamps[-1]  # update the gaze timestamp head, so we can release video frames up to this timestamp

            # up to the point of the last gap filled index, we detect fixation. The idt window ends at the gap filled index
            fixations, last_window_start = fixation_detection_idt(*self.gap_filled_xyz_buffer['gap_filled_xyz'], window_size=self.params['idt_window_size'])
            self.gap_filled_xyz_buffer.clear_up_to(last_window_start)
        ...
        while self.video_stream_name in self.inputs.keys() and len(self.inputs[self.video_stream_name][1]) > 0 and self.inputs[self.video_stream_name][1][0] < self.gaze_timestamp_head:
