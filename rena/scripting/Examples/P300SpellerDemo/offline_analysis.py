...
# row_flashing_index = np.where(colum_row_marker)
print("John")
...
# indexes = [i for i, j in enumerate(merged_list) if j == (ROW_FLASHING_MARKER,4)]
# indexes = [i for i, j in enumerate(merged_list) if j == (ROW_FLASHING_MARKER,4)]
# indexes = [i for i, j in enumerate(merged_list) if j == (ROW_FLASHING_MARKER,4)]
# indexes = [i for i, j in enumerate(merged_list) if j == (ROW_FLASHING_MARKER,4)]
# indexes = [i for i, j in enumerate(merged_list) if j == (ROW_FLASHING_MARKER,4)]
...
#     if item[0]==COL_FLASHING_MARKER:
#         if item[1]==1:
#

# colum_row_index = colum_row_index
# col1_index =
