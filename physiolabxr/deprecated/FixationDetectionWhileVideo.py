...
class FixationDetection(RenaScript):
    ...
    def init(self):
        ...
        self.gap_filled_xyz = DataBuffer()
    ...
    def loop(self):
        ...
        if self.gaze_stream_name in self.inputs.keys():
            ...
            if gaze_status[-1] == self.gaze_status['valid']:  # and starting_invalid_duration > self.max_gap_time:  # if the sequence starts out invalid, we must wait until the end of the invalid
                ...
                self.gap_filled_xyz.update_buffer({'stream_name': 'gap_filled_xyz', 'frames': gap_filled_xyz, 'timestamps': gaze_timestamps})  # add the gap filled data to the buffer, so we can use it for fixation detection
            ...
            # for the point up to the gap filled index, we detect fixation. The idt window ends at the gap filled index
        try:
