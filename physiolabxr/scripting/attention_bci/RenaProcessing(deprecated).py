...
# feature:  the temporal buffer to store the received image

FRAME_HISTORY = 30
right_pull_buffer = deque(maxlen=FRAME_HISTORY)
left_pull_buffer = deque(maxlen=FRAME_HISTORY)
back_pull_buffer = deque(maxlen=FRAME_HISTORY)
fixation_pull_buffer = deque(maxlen=FRAME_HISTORY)
...
class RenaProcessing(RenaScript):
    ...
    def loop(self):
        try:
            ...
            #     left_pull_buffer.append([left_cam_color,left_cam_depth,timestamp_left,bboxes_left])
            #
            # if back_msg:
            #     back_cam_color, back_cam_depth, timestamp_back, bboxes_back = back_msg
            #     back_pull_buffer.append([back_cam_color, back_cam_depth, timestamp_back, bboxes_back])
            #
            # if fixation_msg:
            #     fixation_cam_color, fixation_cam_depth, timestamp_fixation, bboxes_fixation, gaze_x, gaze_y = fixation_msg
            #     fixation_pull_buffer.append([fixation_cam_color, fixation_cam_depth, timestamp_fixation, bboxes_fixation,gaze_x, gaze_y])
            #
            # # test: the OVTR processed capture
            # try:
            #
