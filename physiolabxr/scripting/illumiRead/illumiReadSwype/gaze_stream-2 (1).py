...
# from physiolabxr.scripting.illumiRead.illumiReadSwype.gaze_39 import result
...
def process_fixation_results_to_action_info(fixation_results, action_info, output_csv):
    # Extract the time range from fixation_results
    first_start = min(item['fixation_start'] for item in fixation_results)
    last_end = max(item['fixation_end'] for item in fixation_results)

    # Filter action_info to the relevant time range
    filtered_action_info = action_info[
        (action_info['absoluteTime'] >= first_start) &
        (action_info['absoluteTime'] <= last_end)
    ].copy()

    # Initialize the DesignMatrixCondition column
    filtered_action_info['DesignMatrixCondition'] = 0

    # Mark intervals based on fixation_results
    for fixation in fixation_results:
        start, end, mark = fixation['fixation_start'], fixation['fixation_end'], fixation['mark']
        mask = (filtered_action_info['absoluteTime'] >= start) & (filtered_action_info['absoluteTime'] <= end)
        filtered_action_info.loc[mask, 'DesignMatrixCondition'] = mark

    # Save the DataFrame to a CSV file
    filtered_action_info.to_csv(output_csv, index=False)
...
if __name__ == "__main__":
    ...
    result_dict = process_fixation_results_to_action_info(fixation_results, action_info,"myactioninfo.csv")
