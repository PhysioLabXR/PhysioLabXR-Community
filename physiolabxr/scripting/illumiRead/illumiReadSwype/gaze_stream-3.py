...
def mark_fixation_data_with_design_matrix(action_info, fixation_summary):
    """
    Mark fixation data with possible letter matches and integrate the design matrix method for marking.
    """
    fixation_results = []  # Store fixation data with marking

    # Extract target text for marking
    ...
    for fixation in fixation_summary:
        ...
        if current_word_index >= len(target_text):
            break

        current_target = target_text[current_word_index]
        next_target = target_text[current_word_index + 1] if current_word_index + 1 < len(target_text) else None

        # Check for target matches with possible letters
        if current_target in possible_letters:
            ...
            fixation_results.append({
                ...
                "fixation_end": fixation_end,
                "possible_letters": possible_letters,
                "current_letter": trailing_letter,
                "mark": 2
            })
        ...
        # Advance to the next word or letter as appropriate
        if current_letter_index >= len(current_target):
            current_word_index += 1
            current_letter_index = 0
            trailing_letter = None

        if next_target and parse_two_with_proximity(current_target, next_target, possible_letters):
            fixation_results.append({
                "fixation_start": fixation_start,
                "fixation_end": fixation_end,
                "possible_letters": possible_letters,
                "current_letter": next_target,
                "mark": 2
            })
            current_word_index += 1
...
if __name__ == "__main__":
    ...
    fixation_results = mark_fixation_data_with_design_matrix(action_info,fixation_summary)
    # Final Results
