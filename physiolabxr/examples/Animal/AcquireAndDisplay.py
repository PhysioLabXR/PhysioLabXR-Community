        ...
        while(continue_recording):
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    socket.send_multipart([bytes(topic, "utf-8"), np.ravel(image_data)])
                    print("Sending image")
                    # Draws an image on the current figure
                    # plt.imsave(f"{counter}.png", image_data, cmap='gray')
                    # counter += 1
                    # print("Saved to image '%s'..." % counter)
